...
def print_large_text(message: str, status_code: str, raw_hex: str = "", length: int = 0):
    ...
    if raw_hex:
        print("="*80)
        print(f"{' '*15}Frame Details:")
        print(f"{' '*15}Raw Hex: {raw_hex}")
        print(f"{' '*15}Frame Length: {length} bytes")
    print("="*80)
    ...
    try:
        while True:
            ...
            if result['status_changed']:
                ...
                flag = 0
                print(f"{' '*15} Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
                print("="*80)
...
def clear_console(flag):
    ...
    print("="*80)
    if flag == 1 : print(f"{' '*20} Current Status is") 
    elif flag == 0 : print(f"{' '*20} Status Change Detected!")
